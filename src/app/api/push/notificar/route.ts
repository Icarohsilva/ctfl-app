...
export async function POST(req: NextRequest) {
  ...
  // Busca usuários que NÃO estudaram hoje e têm notificações ativas
  const { data: usuarios } = await supabase
    ...
    .select(`
      ...
      preferencias_notificacao!inner(push_ativo, email_ativo, token_cancelamento)
    `)
  ...
  for (const u of usuarios) {
    const prefs = u.preferencias_notificacao as { push_ativo: boolean; email_ativo: boolean; token_cancelamento: string }[];
    const pref = Array.isArray(prefs) ? prefs[0] : prefs;
    if (!pref) continue;

    const profiles = (Array.isArray(u.profiles) ? u.profiles[0] : u.profiles) as { nome: string };
    const nome = profiles?.nome?.split(" ")[0] || "QA";
    ...
    if (pref.push_ativo) {
      ...
      if (sub?.subscription) {
        try {
          ...
          await supabase.from("push_subscriptions").update({ ativo: false }).eq("user_id", u.user_id);
        }
      }
    }
  ...
  return NextResponse.json({ ok: true, pushEnviados, emailsEnviados });
}
...
async function enviarEmailLembrete(
  ...
  const cancelUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/cancelar-notificacoes?token=${tokenCancelamento}`;
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
              ...
              <table cellpadding="0" cellspacing="0">
                <tr>
                  <td style="background:#1a1a0e;border:1px solid #c9a84c44;border-radius:12px;padding:12px 20px;">
                    <span style="font-size:20px;font-weight:700;color:#e8d5a3;font-family:Georgia,serif;letter-spacing:0.02em;">TestPath</span>
          ...
          <tr>
            <td style="padding:20px 0;">
              <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f0f18;border:1px solid #1e1e2e;border-radius:12px;padding:20px;">
                <tr>
                  <td align="center">
                    <p style="margin:0 0 8px;font-size:13px;color:#5a5a6a;">
                      Lembre-se: a certificação CTFL está ao seu alcance.
                    </p>
          ...
          <tr>
            <td style="padding-top:8px;text-align:center;">
              <p style="margin:0 0 8px;font-size:12px;color:#3a3a4a;">
                Você está recebendo este e-mail porque se cadastrou no TestPath.
              </p>
              <p style="margin:0;font-size:11px;color:#2a2a3a;">
                <a href="${cancelUrl}"
                   style="color:#3a3a4a;text-decoration:underline;">
